...
static s32 i2c_bulkread(HANDLE handle, u8 chip, u16 addr, u8 *data, u16 length)
{
  ...
  {
    fc8080_i2c_read_byte(chip << 1, addr, data, length);
  }
  else
  {
    fc8080_i2c_read_len(chip << 1, addr, data, length);
  }
...
static s32 i2c_bulkwrite(HANDLE handle, u8 chip, u16 addr, u8 *data, u16 length)
{
  if(length == I2C_LENGTH_ONE)
  {
    fc8080_i2c_write_byte(chip << 1, addr, data, length);
  }
  else
  {
    fc8080_i2c_write_len(chip << 1, addr, data, length);
  }
  return BBM_OK;
}
