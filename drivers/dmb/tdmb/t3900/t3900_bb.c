...
void t3700_get_ber(tdmb_bb_sig_type *psigs)
{
  ...
  psigs->RxPowerLevel = INC_GET_RSSI(TMDB_I2C_CTRL_ID);
...
// 110127 (EBI2 : manual Resync, TSIF : HW AutoResync)
  ...
  //TDMB_MSG_INC_BB("snr(%d)pc ber(%d) rs ber(%d) ant(%d) RxPower(%d)\n", (int)psigs->SNR, (int)psigs->PCBER, (int)psigs->RSBER, (int)psigs->RSSI, psigs->RxPowerLevel);
