			      ...
			      struct msm_eeprom_memory_block_t *data)
{
	...
	struct msm_eeprom_memory_map_t *emap = data->map;
	uint8_t *memptr = data->mapdata;
	struct msm_eeprom_board_info *eb_info = NULL;
       ...
       num_block = data->num_map -1; //just check awb,lsc within second block reading time
#elif defined(CONFIG_PANTECH_CAMERA_IMX214)
       num_block = data->num_map - 2; //just check awb,lsc within first block reading time
#endif
       ...
       for (i = 0; i < num_block; i++) { // exeception of af checksum 
              ...
              checksum = get_nvm_checksum(&data->mapdata[data_ptr_index], valid_size);
...
#ifdef CONFIG_PANTECH_CAMERA_EEPROM_CHECKSUM
	rc = read_eeprom_memory_checksum(e_ctrl, &e_ctrl->cal_data);
	if (rc < 0) {
